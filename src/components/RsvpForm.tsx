...
export function RsvpForm({
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (!primaryName) { setError('Please enter your name.'); return }
    if (!attending)   { setError('Let us know if you can make it.'); return }
    setStatus('submitting')
    setError(null)
    const others = String(fd.get('guestNames') ?? '').trim()
    const guestNames = [primaryName, ...others.split('\n').map(n => n.trim()).filter(Boolean)]
    try {
      await submitRsvp({
        data: {
          inviteId,
  ...
  return (
    <form className="bg-parchment border border-amber rounded-3xl p-8 space-y-5" onSubmit={handleSubmit}>
      <Field label="Your name"      name="primaryName" type="text"  placeholder="First and last name" required />
      <label className="block">
        <span className="block uppercase tracking-widest text-xs text-ink/60 mb-1">Others in your party</span>
        <textarea
          name="guestNames"
          rows={3}
      ...
      <fieldset>
        <legend className="uppercase tracking-widest text-xs text-ink/60 mb-3">Will you join us?</legend>
          ...
          {(['yes', 'no'] as const).map((v) => (
            <button
              ...
              className={`flex-1 inline-flex items-center justify-center gap-2 px-4 py-3 border uppercase tracking-widest text-xs rounded-full transition-colors ${
                attending === v ? 'bg-burgundy text-cream border-burgundy' : 'bg-cream text-ink border-amber hover:border-burgundy'
              }`}
            >
              {v === 'yes' ? <><Check className="w-4 h-4" strokeWidth={2.5} /> Accept</> : <><X className="w-4 h-4" strokeWidth={2.5} /> Decline</>}
            </button>
          ))}
        </div>
      </fieldset>
      ...
      {attending === 'yes' && (
        <>
          ...
          <label className="block">
            <span className="block uppercase tracking-widest text-xs text-ink/60 mb-1">Additional notes</span>
            <textarea rows={3} name="notes" className="w-full bg-cream border border-amber focus:border-burgundy rounded-lg outline-none px-3 py-2 text-ink" />
          </label>
...
function PaperLine(props: React.InputHTMLAttributes<HTMLInputElement> & { label: string }) {
  const { label, ...rest } = props
  return (
    <div className="flex items-baseline gap-2 border-b border-ink/20 focus-within:border-ink/50 transition-colors pb-px">
      <span className="shrink-0 text-[9px] md:text-[11px] uppercase tracking-[0.2em] text-ink/35">{label}</span>
      <input
        {...rest}
...
function PaperArea({ label, name }: { label: string; name: string }) {
  ...
  return (
    <div className="border-b border-ink/20 focus-within:border-ink/50 transition-colors pb-px">
      <span className="text-[9px] md:text-[11px] uppercase tracking-[0.2em] text-ink/35 mr-2">{label}</span>
      <textarea
        ref={ref}
        name={name}
        rows={1}
...
function Field(props: React.InputHTMLAttributes<HTMLInputElement> & { label: string }) {
  ...
  return (
    <label className="block">
      ...
      <input {...rest} className="w-full bg-cream border border-amber focus:border-burgundy rounded-lg outline-none px-3 py-2 text-ink" />
    </label>
  )
}
