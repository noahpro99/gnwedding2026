...
// Seed all invite records. Upserts guest_names/party_size_max so re-running the
// generator (bun src/server/invites-gen.ts) propagates CSV changes without
// disturbing existing RSVPs (invite rows are updated, not deleted+reinserted).
const INVITES: Array<{ id: string; guest_names: string; party_size_max: number }> = [
  ...
  { id: "scott-chapman", guest_names: "Scott Chapman, Meghann Chapman, Cora Chapman +1, Marianne Chapman, Luther Chapman, Lori Chapman, Olivia Chapman, Daniel Chapman, Oliver Chapman", party_size_max: 10 },
  ...
  { id: "jacob-mills", guest_names: "Jacob Mills +1", party_size_max: 2 },
