...
export const claimItem = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data }) => {
    try {
      db.run(
        `INSERT INTO registry_claims (item_key, initials) VALUES (?, ?)`,
        [data.itemKey, data.initials],
      )
    } catch (err) {
    ...
    return { ok: true as const, itemKey: data.itemKey, initials: data.initials }
  })
